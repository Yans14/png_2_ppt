...
def render_deck(
    ...
    output_path: str | Path,
    *,
    assets_by_slide: list[dict[str, str]] | None = None,
    node_binary: str = "node",
    renderer_script: str | Path | None = None,
    timeout_seconds: int = 300,
) -> Path:
    """Render multiple editable specs into one native PowerPoint deck."""

    if not specs:
        raise RenderError("At least one slide spec is required")
    first_size = (specs[0].source_width, specs[0].source_height)
    if any((spec.source_width, spec.source_height) != first_size for spec in specs):
        raise RenderError("All slides in one deck must share the same canvas dimensions")
    output = Path(output_path).resolve()
    output.parent.mkdir(parents=True, exist_ok=True)
    node = shutil.which(node_binary)
    if not node:
    ...
    if not script.exists():
        raise RenderError(f"Renderer script not found: {script}")
    assets = assets_by_slide or [{} for _ in specs]
    if len(assets) != len(specs):
        raise RenderError("assets_by_slide must have one mapping per slide")
    envelope = {
        "specs": [spec.model_dump(mode="json") for spec in specs],
        "assets_by_slide": assets,
    }
    with tempfile.TemporaryDirectory(prefix="editable-pptx-render-deck-") as temp_dir:
        spec_path = Path(temp_dir) / "render-deck-spec.json"
        spec_path.write_text(json.dumps(envelope, ensure_ascii=False), encoding="utf-8")
        try:
            result = subprocess.run(
                [node, str(script), str(spec_path), str(output)],
            ...
            raise RenderError(f"PPTX deck rendering timed out after {timeout_seconds}s") from error
    if result.returncode != 0:
        message = (result.stderr or result.stdout or "unknown renderer error").strip()
        raise RenderError(f"PPTX deck rendering failed: {message}")
    if not output.exists() or output.stat().st_size == 0:
        raise RenderError("PPTX deck renderer produced no output")
    _stabilize_text_autofit(output, timeout_seconds)
    return output
